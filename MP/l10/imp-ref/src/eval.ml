...
let rec eval_stmt env s =
  match s with
  | Block (vs,ss)-> let local_env= List.fold_left declare_var env vs 
  in List.iter (eval_stmt local_env) ss
  ...
  | Raise x -> raise (Failure x)
  | Try (s1, x, s2) ->
    try eval_stmt env s1
    with Failure msg -> assign_var env x (int_of_string msg); eval_stmt env s2
