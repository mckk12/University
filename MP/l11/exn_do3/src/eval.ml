...
let eval_op (op : bop) (v1 : value) (v2 : value) : value =
  ...
  | _ -> raise (Type_error "invalid binary operation")
...
let rec eval_env (env : env) (e : expr) : comp =
  ...
  | App (e1, e2) ->
      ...
      | _ -> raise (Type_error "invalid function application"))
  | Pair(e1, e2) ->
      let* v1 = eval_env env e1 in
      let* v2 = eval_env env e2 in
      return (VPair(v1, v2))
  | Fst e ->
      ...
      | _ -> raise (Type_error "invalid fst operation"))
  | Snd e ->
      let* v = eval_env env e in
      (match v with
      | VPair(_, v2) -> return v2
      | _ -> raise (Type_error "invalid snd operation"))
  | Raise -> Exn
  | Try(e1, e2) ->
