...
type hstack = cmd list* vm_value list* int
...
let exec_prog p =
  ...
  let rec exec (p : cmd list) (env : vm_value list) (hstack: hstack): vm_value =
    match p with
    | [] -> pop ()
    ...
    | Return :: _ ->
      ...
      | _ -> failwith "error")
    | BeginTry(p1, q) :: p' -> 
      exec p1 env ((q,env,!sp)::hstack)
    | EndTry :: p' -> exec p' env (List.tl hstack)
    | Raise :: _ -> 
      (match hstack with
      | (q,env',sp')::hs -> 
        sp := sp';
