...
let rec string_of_value v =
  ...
  | VClosure _  -> "<fun>"
  | VPair(v1, v2) ->
    let str_v1 = match v1 with
      | VPair _ | VCtor _ -> "(" ^ string_of_value v1 ^ ")"
      | _ -> string_of_value v1
    in
    let str_v2 = match v2 with
      | VPair _ | VCtor _ -> "(" ^ string_of_value v2 ^ ")"
      | _ -> string_of_value v2
    in
    str_v1 ^ ", " ^ str_v2
  | VCtor(c, v) ->
    let str_v = match v with
      | VPair _ | VCtor _ -> "(" ^ string_of_value v ^ ")"
      | _ -> string_of_value v
    in
    c ^ " " ^ str_v

(*Zmieniamy funkcję string_of_value tak, aby nie otaczała wartości nawiasami w przypadku par i konstruktorów.
Dodajemy nawiasy tylko wtedy, gdy wartość jest parą lub konstruktorem. *)
